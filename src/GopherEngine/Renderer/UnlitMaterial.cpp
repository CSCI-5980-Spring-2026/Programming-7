...
namespace GopherEngine {
    ...
    void UnlitMaterial::draw(const Geometry& geometry, const glm::mat4 &world_matrix, const RenderContext& context) {
        ...
        // and allows us to access mesh-specific data like vertices and indices.
        ...
        // Lazy upload to GPU on first draw, return if upload fails
        if(mesh.vao_ == 0 && !ResourceUploader::upload_mesh(mesh))
            return;

        // Lazy load the texture to GPU if it hasn't been uploaded yet
        bool use_texture = false;
        if(texture_) {
            if(texture_->texture_id_ == 0)
                use_texture = ResourceUploader::upload_texture(*texture_);
            else
                use_texture = true;
        }
